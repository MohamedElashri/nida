...
func ResolvePermalink(item content.Item, cfg config.SiteConfig) (string, error) {
	...
	pattern, err := patternFor(item.Type, cfg)
	if err != nil {
		return "", err
	}
...
func patternFor(itemType string, cfg config.SiteConfig) (string, error) {
	switch itemType {
	case content.TypePost:
	...
	default:
		return "", fmt.Errorf("resolve permalink: unsupported content type %q", itemType)
	}
}
